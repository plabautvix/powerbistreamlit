...
class Examples:
    ...
    def line_chart(self):
        fig = px.line(
            self.df, title="Line Chart", labels={"index": "Index", "value": "Value"}
        )
        st.plotly_chart(fig)

    def pie_chart(self):
        fig = px.pie(
            self.category_df, names="Category", values="Values", title="Pie Chart"
        )
        st.plotly_chart(fig)
    ...
    def heatmap(self):
        fig = go.Figure(
            data=go.Heatmap(
                z=self.df.corr().values, x=self.df.columns, y=self.df.columns
            )
        )
        fig.update_layout(
            title="Heatmap", xaxis_title="Features", yaxis_title="Features"
        )
        st.plotly_chart(fig)
    ...
    def line_area_combined(self):
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(x=self.df.index, y=self.df["A"], mode="lines", name="Line")
        )
        fig.add_trace(
            go.Scatter(x=self.df.index, y=self.df["B"], fill="tozeroy", name="Area")
        )
    ...
    def treemap(self):
        fig = px.treemap(
            self.category_df, path=["Category"], values="Values", title="Treemap"
        )
        st.plotly_chart(fig)
